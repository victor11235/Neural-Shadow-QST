...
class ShadowTomographyTrainer():
    ...
    def _step(self, i=0):
        ...
        for cliff, res in zip(cliffords_batch, results_batch):
            ...
            loss_vmc_i = - 2 * (2**self.num_qubits+1) * loss_real / self.batch_size
            loss_vmc += loss_vmc_i
        
        # end of the step, we update the model parameters
        #torch.nn.utils.clip_grad_norm_(self.nqs.parameters(), 2)
        loss_vmc.backward()
        self.optimizer.step()
    ...
    def train(self):
        ...
        for i in range(self.max_iters):
            logging_dict, cost_fun = self._step(i=i)           
